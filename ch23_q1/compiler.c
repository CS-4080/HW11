...
static void switchStatement() {
  ...
  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF)) {
    if (match(TOKEN_CASE)) {
      // Duplicate the switch value by re-reading it from the top of stack.
      // Stack before case compare: [..., switchValue]
      emitByte(OP_DUP); // if you already added OP_DUP; otherwise see note below

      expression();     // compile case value
      consume(TOKEN_COLON, "Expect ':' after case value.");

      emitByte(OP_EQUAL);
      int nextCaseJump = emitJump(OP_JUMP_IF_FALSE);
      emitByte(OP_POP); // pop comparison result when true

      // Compile statements for this case until next case/default/}
      ...
      emitByte(OP_POP); // pop comparison result when false
    } else if (match(TOKEN_DEFAULT)) {
      if (hasDefault) {
        error("Already have a default case in switch.");
      }
      hasDefault = true;
